...
\textbf{Related work.} Kirchenbauer~\etal~\cite{kirchenbauer2023} introduced the green-list logit-bias watermark, evaluated on OPT-1.3B and LLaMA-1; a follow-up~\cite{kirchenbauer2024reliability} re-tested it under adversarial paraphrasing and human rewriting. Distortion-free alternatives~\cite{kuditipudi2023robust} preserve the model distribution but require aligned-decoding detectors, and~\cite{christ2024undetectable} proved an information-theoretic detectability bound. We focus on the logit-bias scheme because it admits a closed-form $z$-test detector and ships as a HuggingFace \texttt{LogitsProcessor} without model retraining.
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.75\linewidth]{figures/fig1_method.png}
...
\caption{\textbf{Detectability/quality tradeoff in $\delta$, Gemma 2 9B.} Red: TPR at $1\%$ FPR (frozen threshold from $\delta{=}2$ unwatermarked). Blue: GPT-2 perplexity ratio of watermarked over unwatermarked completions on the same prompt subset. Knee at $\delta{=}2$.}
\label{fig:delta}
\end{figure}

\subsection{Cross-architecture check on LLaMA 3.1 8B}
\label{sec:cross}
The same code path, with only the model identifier changed, was applied to LLaMA~3.1 8B Instruct. We generated 197 completions ($99$ watermarked, $98$ control) at $\delta{=}2$ and ran the identical detection eval. Detection is in fact stronger on LLaMA than on Gemma: $\mathrm{TPR}@1\%\mathrm{FPR}=98.0\%$ on the full corpus (threshold $\tau{=}2.20$ calibrated on $n_\text{uwm}{=}98$, with measured FPR $0/98$), and the GPT-2 perplexity ratio is $1.01$, so watermarked and unwatermarked are statistically indistinguishable on quality. Robustness is also stronger: $96\%$ TPR survives $10\%$ word substitution and $91\%$ survives $20\%$ substitution, versus $84\%$ and $75\%$ on Gemma. Under a full LLM-paraphrase attack in which LLaMA itself rewrites each watermarked completion (no logit processor active), TPR collapses to $27.3\%$ (mean $z{=}1.20$). The watermark therefore survives surface edits comfortably but is reliably evaded by a determined paraphraser, matching the qualitative claim of~\cite{kirchenbauer2024reliability} and motivating the distortion-free and semantic-grouping variants we cite as future work. Figure~\ref{fig:cross} overlays the two length curves; LLaMA and Gemma track each other within a few percentage points across all length bins, with LLaMA slightly above. The tokenisers differ in size ($128k$ vs.\ $256k$) but $\gamma{=}0.5$ keeps the green-list density invariant, and the SHA-256 keying makes the partition independent of vocabulary identity.
